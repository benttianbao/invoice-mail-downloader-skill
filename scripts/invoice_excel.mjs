...
sheet.getRange("A2").values = [["浅蓝色为自动填写字段；浅黄色为人工填写字段；费用类型请从下拉列表选择。"]];
...
const rows = records.map((record, index) => {
  ...
  return [
    ...
    manual ? manual.paymentDate : null,
    manual ? manual.recipient : "",
    amountValue(record.invoice_amount),
    manual ? manual.reimbursementAmount : amountValue(record.invoice_amount),
...
if (rows.length) {
  ...
  sheet.getRange(`A4:M${rows.length + 3}`).values = rows;
}
...
const note = sheet.getRange("A2:L2");
note.format = {
  fill: "#E6FFFA",
...
const header = sheet.getRange("A3:M3");
header.format = {
  fill: "#319795",
...
if (rows.length) {
  const data = sheet.getRange(`A4:M${lastDataRow}`);
  data.format = {
    verticalAlignment: "center",
  ...
  sheet.getRange(`K4:L${lastDataRow}`).format.wrapText = true;
  ...
  for (const column of ["A", "B", "E", "H", "I", "J", "L"]) {
  ...
  const table = sheet.tables.add(`A3:L${lastDataRow}`, true, "InvoiceRegisterTable");
...
if (rows.length) {
  ...
  const statusRange = sheet.getRange(`L4:L${lastDataRow}`);
  statusRange.conditionalFormats.addCustom('=LEFT($L4,3)="待确认"', { fill: "#FED7D7", font: { color: "#9B2C2C", bold: true } });
  statusRange.conditionalFormats.addCustom('=$L4="通过"', { fill: "#C6F6D5", font: { color: "#22543D", bold: true } });
}
...
sheet.getRange(`M1:M${lastDataRow}`).format.font = { color: "#FFFFFF", size: 1 };
...
console.log(JSON.stringify({ output: outputPath, rows: records.length, inspection: inspection.ndjson ?? inspection }, null, 2));
